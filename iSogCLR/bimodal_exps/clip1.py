...
def main(args):
            ...
            if args.evaluate:                
                ...
                log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                            #  **{f'val_{k}': v for k, v in val_result_coco.items()},
                             ...
                             'r_mean_val_cc3m_10': val_result_coco['r_mean'] ##################################
                            }
                with open(os.path.join(args.output_dir, "coco_log.txt"),"a") as f:
                    f.write(json.dumps(log_stats) + "\n")
    ...
    if utils.is_main_process():   
        with open(os.path.join(args.output_dir, "coco_log.txt"),"a") as f:
            f.write(f"best epoch: {best_epoch} Best r_mean: {best}")            
...
if __name__ == '__main__':
    ...
    args.coco_image_root = os.path.join(args.data_path, args.train_image_root)
