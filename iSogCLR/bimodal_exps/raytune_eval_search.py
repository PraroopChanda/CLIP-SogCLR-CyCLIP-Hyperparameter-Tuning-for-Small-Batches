...
def main_training_func(raytune_config, base_args):
    import warnings
    def warn(*args, **kwargs):
        pass
    warnings.warn = warn
    ...
    args = copy.deepcopy(base_args)
        ...
        #args should contain the ray_tune params
    for key, value in raytune_config.items():
        if hasattr(args, key):
            setattr(args, key, value)

    #avoid any write conflicts
    random_string = f'{int(time.time())}_{random.randint(0, 10000)}'
    args.checkpoint = f'{args.checkpoint}_{random_string}'
    args.output_dir = f'{args.output_dir}_{random_string}'
    ...
    for epoch in range(0, max_epoch):
        ...
        if utils.is_main_process():  
            ...
            if args.evaluate:                
                ...
                ##################################
                # ray_train.report({"loss": float(train_stats["loss_ita"]), "avg_image_tau": float(train_stats["avg_image_tau"]),
                # "avg_text_tau": float(train_stats["avg_text_tau"])})
                if args.train_and_val:
                    ray_train.report({"val_acc": float(val_result_coco["r_mean"]), "train_loss": float(train_stats["loss_ita"]), 
                                    "avg_image_tau": train_stats["avg_image_tau"], "avg_text_tau": train_stats["avg_text_tau"]})
                ##################################
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_file', default='cc3m_train_new.json')
    ...
    # output path
    parser.add_argument('--output_dir', default='../dump_temp/output/clip')  

    # loss config
    parser.add_argument('--ita_type', default='clip',required=True, choices=['clip', 'cyclip', 'vicreg', 'sogclr', 'sogclr_dro', 
    ...
    #specify all the params explicitly here for raytune setup


    raytune_config = {}

    if args.ita_type == "clip":
        #lr, softmax_temp
        num_samples=5
        raytune_config = {
            "lr": ray_tune.loguniform(1e-5, 1e-2),
            "temp": ray_tune.loguniform(1e-3, 1e-1)
        }
    elif args.ita_type == "sogclr":
        num_samples=10
        raytune_config = {
            "lr": ray_tune.loguniform(1e-5, 1e-2),
            "sogclr_gamma": ray_tune.loguniform(0.5,0.9),
            "temp": ray_tune.loguniform(1e-3, 1e-1)
        }
        
    elif args.ita_type == "cyclip":
        num_samples=10
        raytune_config = {
            "lr": ray_tune.loguniform(1e-5, 1e-2),
            "temp": ray_tune.loguniform(1e-3, 1e-1),
            "cylambda_1": ray_tune.uniform(0.25,0.75),
            "cylambda_2": ray_tune.uniform(0.25,0.75)
        }        
    else:
        pass
    ...
    # search_space = {
    # "lr": ray_tune.loguniform(1e-5, 1e-2),
    # "sogclr_gamma": ray_tune.uniform(0.5, 0.9),
    # "rho_I": ray_tune.uniform(4.0, 12.0),
    # "rho_T": ray_tune.uniform(4.0, 12.0),
    # "tau_init": ray_tune.loguniform(1e-3, 1e-1),
    # "eta_init": ray_tune.loguniform(1e-4, 1e-2),
    # "beta_u": ray_tune.uniform(0.8, 0.99),
    # "temp": ray_tune.loguniform(1e-3, 1e-1)
    # }
    #################
    ...
    args.val_coco_file = os.path.join(args.ann_path, 'cc3m_train_subset_10.json')
    args.coco_image_root = args.train_image_root ################
    ...
    # tuner = ray_tune.Tuner(partial(main_training_func, base_args=args),
    #                        tune_config=ray_tune.TuneConfig(num_samples=10, scheduler=scheduler, search_alg=hyperopt_search),
    #                        ,ray_tune.with_resources(ray_tune.with_parameters(objective),resources={"cpu": 2, "gpu": 1},param_space=raytune_config)
    tuner = ray_tune.Tuner(ray_tune.with_resources(ray_tune.with_parameters(partial(main_training_func, base_args=args)), resources={"cpu": 8, "gpu": 1}),
                           tune_config=ray_tune.TuneConfig(num_samples=num_samples, scheduler=scheduler, search_alg=hyperopt_search),
                           param_space=raytune_config)  
